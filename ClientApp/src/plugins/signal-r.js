...
class SocketConnection extends events {
  ...
  listen(method) {
    ...
    this.on('init', () => {
      this.socket.on(method, (...args) => {
        if (this.options.log) console.log({ type: 'receive', method, args });
...
export default {
  install: (app, options) => {
    const Socket = new SocketConnection(options.connection);
    // app.socket = Socket;
