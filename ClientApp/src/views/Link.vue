<template>
  <div>
    ...
    <div class="
      ...
      <Modal :showing="showSelected" @close="closeSelected" :showClose="true">
        <Assign :dispatch="selected" :updateDispatch="updateDispatch" :products="products" />
      </Modal>
    </div>
  </div>
</template>
...
export default {
  ...
  async mounted() {
    this.$store.commit("dispatches/setCurrentQuery", "getPending");
    this.$store.dispatch("dispatches/getPending");
    ...
    this.products = products.filter(item => item != null || item.name != null || item?.name != "")
  },
  data() {
    return {
      showWeigh: false,
