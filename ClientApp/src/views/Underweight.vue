      ...
      <table class="text-left table-collapse w-full">
        <thead class="before:border">
          <tr class="bg-gray-100 border-b border-t border-border-gray-900">
            ...
            <th class="td">Nett (kg)</th>
            <th class="td"></th>
          </tr>
        </thead>
        <tbody class="align-baseline">
          <tr
            v-for="dispatch in filteredUnderweights()"
...
export default {
  ...
  inject: ["authHeaders"],
  async mounted() {
    this.underweights = await fetch(
      `${import.meta.env.VITE_API}/api/dispatches/underweights`,
